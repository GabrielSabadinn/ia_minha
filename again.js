...
app.post('/api/chatbot/responder', async (req, res) => {
  ...
  if (!user_input) {
    ...
    // Executar consulta SQL aqui e responder com o resultado
    if (normalized_input.match(/\bncm\b/i)) {
      // Lógica para NCM
      const ncmMatch = user_input.match(/ncm (\d{4}[\s.]\d{2}[\s.]\d{2}) (no estado de |de |do |da )?([A-Za-z]{2})/i
      );
      
      if (ncmMatch && ncmMatch.length === 4) {
        const ncm = ncmMatch[1].replace(/\s/g, '.'); // Formatar NCM
        const uf = ncmMatch[3].toUpperCase(); // Converter UF para maiúsculas
        console.log('NCM:', ncm);
        console.log('UF:', uf);
      
        if (uf.toUpperCase() === 'ES') {
          // Lógica específica para o Espírito Santo (ES)
          const resposta = `O Espírito Santo tem uma alíquota especial de ICMS para a NCM ${ncm}. Aqui estão os detalhes específicos para o ES...`;
          ...
          // Adicione aqui a consulta SQL para outros estados, se necessário
          const consultaSQL = `
          ...
          try {
            ...
            if (result.recordset.length > 0) {
              const resposta = `A alíquota de ICMS para a NCM ${result.recordset[0].ncm} no estado ${result.recordset[0].uf} é ${result.recordset[0].aliquota}%. Lembre-se de verificar com um especialista para informações atualizadas.`;
              historico.push({ "pergunta": user_input, "resposta": resposta });
              await fs.writeFile('historico_perguntas_respostas.json', JSON.stringify(historico, null, 4), 'utf-8');
              return res.json({ "resposta": resposta });
            } else {
              return res.status(404).json({ "error": "Não foi possível encontrar informações para a NCM e UF fornecidas." });
            }
          } catch (err) {
  ...
  // ...----------------------------------------------------------------------------------------
  if (normalized_input.match(/\bmva\b/i) && normalized_input.match(/\bcest\b/i)) {
    const cestMatch = user_input.match(/cest\s*(\d{2}\.\d{3}\.\d{2})/i);
    const ufMatch = user_input.match(/estado\s*(de|do|da)?\s*([A-Za-z]{2})/i);

    if (cestMatch && cestMatch.length === 2) {
        const cest = cestMatch[1];
        const uf = ufMatch && ufMatch.length === 3 ? ufMatch[2].toUpperCase() : null;

        if (uf) {
            // Consulta dinâmica à API ou à fonte de dados para obter a MVA
            const apiUrl = `https://www.lefisc.com.br/api/st/chatbot/pesquisa-mva?cest=${cest}&sigla=${uf}`;
            ...
            try {
                ...
                if (apiResponse.status === 200) {
                    ...
                    if (responseData.status === 200) {
                        const respostaFinal = responseData.message;

                        historico.push({ "pergunta": user_input, "resposta": respostaFinal });
                        await fs.writeFile('historico_perguntas_respostas.json', JSON.stringify(historico, null, 4), 'utf-8');
                        return res.json({ "resposta": respostaFinal });
                    } else {
                        return res.status(responseData.status).json({ "error": responseData.message });
                    }
                } else {
                    return res.status(apiResponse.status).json({ "error": "Erro ao chamar a API." });
                }
            } catch (error) {
                console.error('Erro ao chamar a API:', error);
                return res.status(500).json({ "error": "Erro interno do servidor ao chamar a API." });
            }
        } else {
            return res.status(400).json({ "error": "Não foi possível extrair a UF da pergunta." });
        }
    } else {
        return res.status(400).json({ "error": "Não foi possível extrair a CEST da pergunta." });
    }
}
...
// -----------------------------------------------------------------------
  ...
  if (
    normalized_input.match(/\bipi\b/i) &&
    normalized_input.match(/\bpis\/cofins\b/i) &&
    normalized_input.match(/\bncm\b/i)
  ) {
    console.log('Pergunta contém palavras-chave para NCM.');
  
    const ncmMatch = user_input.match(/ncm\s*([\d.]+)/i);
  
    if (ncmMatch && ncmMatch.length > 1) {
      console.log('NCM Match:', ncmMatch);
  
      let ncm = ncmMatch[1].replace(/[^\d]/g, '');
  
    
      if (ncm.length === 8) {
        console.log('NCM:', ncm);
  
        const apiUrl = 'https://www.lefisc.com.br/api/mapatributario/ListaFederalNCMS/1551094d-975e-4fec-8c6a-c37946a23717';
  
        try {
          const apiResponse = await axios.post(apiUrl, { ncm: [ncm] });
          ...
          if (apiResponse.status === 200) {
            ...
            if (responseData.length > 0) {
              const resultados = responseData.map(item => ({
                ipi: item._ipi[0].ipi,
                piscConfins: item._piscofins[0].piscofins
              }));
  
              const respostaFinal = { "resposta": resultados };
  
              console.log('Resposta Final:', respostaFinal);
  
              const historico = await fs.readFile('historico_perguntas_respostas.json', 'utf-8').then(JSON.parse).catch(() => []);
              historico.push({ "pergunta": user_input, "resposta": respostaFinal });
              await fs.writeFile('historico_perguntas_respostas.json', JSON.stringify(historico, null, 4), 'utf-8');
  
              return res.json(respostaFinal);
            } else {
              return res.json({ "error": "Nenhuma informação encontrada para a NCM fornecida." });
            }
          } else {
            return res.json({ "error": "Erro ao chamar a API." });
          }
        } catch (error) {
          console.error('Erro ao chamar a API:', error);
          return res.json({ "error": "Erro interno do servidor ao chamar a API." });
        }
      } else {
        return res.json({ "error": "Formato inválido para a NCM na pergunta. Espera-se 8 dígitos." });
      }
    } else {
      return res.json({ "error": "Não foi possível extrair a NCM da pergunta." });
    }
  }
